...
rsconnect::setAccountInfo(name='betris',
                          token='2BC1213672B9C673065EF138B479A806',
                          secret='su9u3Vv8/kO8mTMzgPhO48PZa/WNa22S5AZwf78r')
...
summary_HD <- data_final %>% 
  filter(trimws(tolower(Cholestrol_Cat1)) == "high") %>%
  count(HeartDisease) %>%
  mutate(perc = if(sum(n) > 0) n / sum(n) * 100 else 0)
...
server <- function(input, output, session) {
  ...
  output$choldis <- renderPlotly({
    ...
    plot_ly(filtered_data, x = ~Cholestrol_Cat1, y = ~n, type = 'bar',
            color = ~Cholestrol_Cat1, colors = custom_c) %>%
      layout(
        title = "Amount of Patient with Cholesterol",
        xaxis = list(title = "Cholesterol Category"),
        yaxis = list(title = "Amount of Patient")
      )
  })
  
  output$heatmap <- renderPlotly({
    plot_ly(
      x = colnames(order_corr),
      y = rownames(order_corr),
      z = order_corr,
      type = "heatmap",
      colorscale = list(c(0, "#ffcccc"), c(1, "#f44336"))
    ) %>%
      layout(
        title = "Factor that Cause Heart Disease",
        xaxis = list(title = "Heart Disease Factors", tickangle = 20),
        yaxis = list(title = "Heart Disease Factors")
      )
  })
